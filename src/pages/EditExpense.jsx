...
export default function EditExpense() {
  ...
  const [form, setForm] = useState(() => {
    ...
    return {
      ...
      // activity
      activityLocation: d.location || '',
      activityTime: d.time || '',
      // car_rental
      carCompany: d.company || '',
      carPickup: d.pickup || '',
      carReturn: d.return || '',
      pricePerDay: d.pricePerDay || '',
      // misc
      miscSub: d.subcategory || 'Alimentação',
    }
  })
  ...
  if (!expense) return (
    <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center">
      <p className="text-slate-400">Gasto não encontrado.</p>
    </div>
  )
  ...
  function set(field, value) {
    setForm(f => {
      ...
      if (field === 'pricePerDay' || field === 'carPickup' || field === 'carReturn') {
        ...
        const d1 = updated.carPickup
        const d2 = field === 'carReturn' ? value : updated.carReturn
        if (pd && d1 && d2 && d2 >= d1) {
  ...
  const showGallery = form.category === 'flight' || form.category === 'car_rental'
  const galleryLabel = form.category === 'flight' ? 'Cartão de embarque' : 'Reserva / contrato'
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-4 pb-10">
        ...
        {showGallery && (
          <ImageGallery expenseId={expenseId} label={galleryLabel} />
        )}
        ...
        <Button type="submit" size="lg" className="w-full mt-4">Salvar Alterações</Button>
      </form>
    </div>
  )
}
